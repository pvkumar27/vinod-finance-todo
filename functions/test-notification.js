...
async function checkTodos() {
  try {
    console.log('Checking todos in the database before testing...');
    ...
    if (todosSnapshot.empty) {
      console.log('No incomplete todos found in the database');
      return;
    }
    
    console.log(`Found ${todosSnapshot.size} incomplete todos in the database:`);
    ...
    console.log(`  Total incomplete tasks: ${todosSnapshot.size}`);
    
    if (overdueCount === 0) {
      console.log('\nWARNING: No overdue tasks found. The notification may only include today\'s tasks.');
      console.log('To add an overdue task, run: node fix-todos-structure.js');
    }
...
async function testNotification() {
  // First check todos
  const expectedTaskCount = await checkTodos();
  ...
  console.log(`\nTesting notification function with URL: ${url}`);
