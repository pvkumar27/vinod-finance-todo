  ...
  return (
    <div
      ...
      className={`
        ...
        ${isPinned ? 'bg-yellow-50 border-l-2 border-yellow-400' : 'bg-blue-50'}
      `}
      {...attributes}
    >
      {/* Checkbox */}
      ...
      <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center pl-1 pr-0.5" {...listeners}>
      ...
      <div className="flex flex-row items-center flex-shrink-0 ml-auto mr-0 space-x-0">
        <button
          type="button"
          onClick={() => onTogglePin(task.id, task.pinned)}
          className={`w-3.5 h-3.5 sm:w-6 sm:h-6 flex items-center justify-center transition-colors ${
            isPinned 
              ? 'text-yellow-600' 
              : 'text-gray-400'
          }`}
        ...
        <button
          type="button"
          onClick={() => onEdit(task)}
          className="w-3.5 h-3.5 sm:w-6 sm:h-6 flex items-center justify-center text-gray-400"
          aria-label={`Edit task: ${task.task}`}
        >
          <svg className="w-1.5 h-1.5 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
