...
`;
      ...
      if (data.error) {
        console.log('⚠️ Gemini API error:', data.error);
  ...
  async executeAction({ action, params }) {
    ...
    switch (action) {
      ...
      case 'get_insights':
        ...
        // Todo insights
        const overdueTodos = allTodos.filter(t => !t.completed && t.due_date && new Date(t.due_date) < new Date());
        ...
        const inactiveCards = allCards.filter(c => {
          ...
          const daysSince = Math.floor((new Date() - new Date(c.last_used_date)) / (1000 * 60 * 60 * 24));
          return daysSince > 90;
        });
  ...
  async fallbackProcess(query) {
    console.log('🔧 Using fallback processing for:', query);
    const lowerQuery = query.toLowerCase();
    
    // Handle UI operations first (before todo/card processing)
    if (lowerQuery.includes('switch') && (lowerQuery.includes('table') || lowerQuery.includes('card') || lowerQuery.includes('view'))) {
    ...
    if (lowerQuery.includes('todo') || lowerQuery.includes('task')) {
      ...
      } else if (lowerQuery.includes('update') || lowerQuery.includes('change') || lowerQuery.includes('mark') || lowerQuery.includes('complete') || lowerQuery.includes('move') || lowerQuery.includes('pin') || lowerQuery.includes('unpin')) {
        // Handle date-based moves first
        if (lowerQuery.includes('move') && (lowerQuery.includes('today') || lowerQuery.includes('tomorrow'))) {
    ...
    if (lowerQuery.includes('mark') || lowerQuery.includes('complete')) {
      return {
        success: false,
        message: 'To mark a todo as completed, try: "mark [task name] as completed" or "complete [task name]"',
        processingMode: 'fallback'
      };
    }
    ...
    throw new Error('I can help with todos and credit cards. Try: "show my todos" or "show my credit cards"');
  }
